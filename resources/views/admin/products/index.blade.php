                            ...
                            <ul class="container-fluid">
                                @foreach ($products as $product)
                                    <li class="row row-cols-2 my-1">
                                        <a href="{{ route('admin.products.show', $product->slug) }}" title="View product"
                                            class="col-10">
                                            {{ $product->id }} {{ $product->name }} <span class="text-capitalize brand"
                                                title="Brand: {{ $product->brand->name }}">{{ $product->brand->name }}</span>

                                            @if ($product->type_id && $product->texture_id)
                                                <sub class="text-capitalize">
                                                    <span
                                                        title="Type {{ Str::lower($product->type->name) == $product->texture->name ? '& Texture' : '' }}">{{ $product->type->name }}</span>
                                        ...
                                        <div class="edit col-2 row row-cols-2">
                                            <a href="{{ route('admin.products.edit', ['product' => $product->slug]) }}"
                                                class="btn btn-primary col-auto"><i class="fa-solid fa-pencil"></i></a>
                                            <form
                                                action="{{ route('admin.products.destroy', ['product' => $product->slug]) }}"
                                                method="post">
                                                @csrf
